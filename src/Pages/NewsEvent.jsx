import { Pdfdata, NewsdataTest } from "../PdfData";
      ...
      <div className="flex gap-8">
        ...
        <div className="flex-1">
          <div className="flex flex-col sm:gap-5 gap-3">
            <h2 className="lg:text-[32px] sm:text-2xl text-xl roboto-serif-font font-semibold text-primary text-left ">
              News, Events and Testimonials
            </h2>
            <h3 className="text-primary sm:text-xl text-sm font-semibold roboto-serif-font">
              Various In-house and on-site training feedback and testimonials
            </h3>
          </div>
          <ul className="flex flex-col lg:gap-[50px] gap-8 mt-10">
            {NewsdataTest.slice(0, 43).map((data, index) => (
              <li key={index} className="flex bg-white">
                <Link to="/newsinner" className="lg:text-xl sm:text-lg text-sm roboto-serif-font text-blacklight">
                  {data.title}
                </Link>
              </li>
            ))}
