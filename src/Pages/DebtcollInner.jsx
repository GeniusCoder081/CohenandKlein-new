...
const DebtcollInner = () => {
  ...
  const [selectedIndex, setSelectedIndex] = useState(0); // Default first option
  const selectedData = DebtCollData[selectedIndex];
  ...
  const handleSelection = (selectedItemIndex) => {
  ...
  const handleNext = () => {
    if (selectedIndex < DebtCollData.length - 1) {
      setSelectedIndex((prevIndex) => prevIndex + 1);
    }
  };

  // Truncate Titles for Dropdown List & Button
        ...
        <div>
          ...
          <div className="2xl:px-15 lg:px-15 sm:px-8 pb-8 px-6 flex flex-col justify-center 2xl:gap-12 md:gap-8 gap-6">
            ...
            <div className="relative inline-block text-left" ref={dropdownRef}>
              ...
              >
                <span>{truncateTitle(selectedData.title)}</span> <FaAngleDown />
              </button>
              ...
              {isOpen && (
                <div className="absolute mt-2 w-full sm:h-[500px] h-[370px] bg-white border border-gray-300 rounded-lg shadow-lg overflow-y-auto custom-scrollbar">
                  ...
                  <div className="px-4 py-3">
                    ...
                    Debt Collection Training Testimonials
                    </h3>
                  </div>

                  {/* Dropdown Items */}
                  {DebtCollData.map((item, idx) => (
                    <button
                      key={item.id}
          ...
          {/* <div className="2xl:px-[130px] lg:px-15 sm:px-8 lg:py-25 sm:py-15 py-10 px-6 flex flex-wrap justify-center gap-2">
            ...
            {Array.from({ length: DebtCollData.length })
              .slice(
                Math.max(0, selectedIndex - 2),
                Math.min(DebtCollData.length, selectedIndex + 3)
              )
              .map((_, i) => {
                const pageIndex =
                  selectedIndex <= 1
                    ? i
                    : selectedIndex >= DebtCollData.length - 2
                    ? DebtCollData.length - 5 + i
                    : selectedIndex - 2 + i;

                if (pageIndex < 0 || pageIndex >= DebtCollData.length)
                  return null;
            ...
            <button
              onClick={handleNext}
              disabled={selectedIndex === DebtCollData.length - 1}
              className="bg-primary text-white px-4 py-2 rounded-md disabled:opacity-50 flex items-center"
            >
          ...
          <div className="2xl:px-[130px] lg:px-15 sm:px-8 lg:py-25 sm:py-15 py-10 px-6 flex flex-wrap justify-center gap-2">
            ...
            <button
              onClick={() => {
                ...
                window.scrollTo({ top: 0, behavior: "smooth" }); // just added scroll
              }}
              disabled={selectedIndex === 0}
              className="bg-primary text-white px-4 py-2 rounded-md disabled:opacity-50 flex items-center"
            >
